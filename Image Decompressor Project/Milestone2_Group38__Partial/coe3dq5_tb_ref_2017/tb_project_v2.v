...
`define OUTPUT_FILE_NAME "motorcycle_tb.ppm"//for M1 is "motorcycle_tb.ppm"

// file for comparison
// to test milestone 2 independently, use the .sram_d1 file to check the output
`define VERIFICATION_FILE_NAME "motorcycle.sram_d1"//for M1 is "motorcycle.sram_d0"
...
//				warn_writing_out_of_region = warn_writing_out_of_region + 1;
		...
		if (SRAM_ARRAY[uut.SRAM_address] != uut.SRAM_write_data) begin
			$write("error: wrote value (%x,%x) to location %d (%x hex), should be value (%x,%x)\n",
				uut.SRAM_write_data[15:8], uut.SRAM_write_data[7:0], uut.SRAM_address, uut.SRAM_address,
				SRAM_ARRAY[uut.SRAM_address][15:8], SRAM_ARRAY[uut.SRAM_address][7:0]);
			//$write("sim time %t\n", $realtime);
			//$write("print some useful debug info here...\n");
			//$write("m1 state %d\n", uut.M1_unit.M1_state);
